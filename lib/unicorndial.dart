  ...
  Widget returnLabel() {
    return Container(
        ...
        child: Text(this.labelText!,
            style: TextStyle(
                fontSize: this.labelFontSize, fontWeight: FontWeight.bold, color: this.labelColor == null ? Color.fromRGBO(119, 119, 119, 1.0) : this.labelColor)));
  }
...
class _UnicornDialer extends State<UnicornDialer> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    this._parentController = AnimationController(vsync: this, duration: Duration(milliseconds: widget.mainAnimationDuration));
  ...
  Widget build(BuildContext context) {
    ...
    var mainFAB = AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) {
          return Transform(
              transform: new Matrix4.diagonal3(vector.Vector3(_parentController.value, _parentController.value, _parentController.value)),
                ...
                child: Row(children: [
                  ScaleTransition(
                      ...
                      child: (!widget.childButtons![index].hasLabel) || widget.orientation == UnicornOrientation.HORIZONTAL
                          ? Container()
                          : Container(padding: EdgeInsets.only(right: widget.childPadding), child: widget.childButtons![index].returnLabel())),
                  ScaleTransition(
                      scale: CurvedAnimation(
                        parent: this._animationController,
                        curve: Interval(intervalValue, 1.0, curve: Curves.linear),
